...
void Init(struct android_app* app)
{
    ...
    // Load custom font from embedded C array (font.h, zt.ttf)
    // font_v is read-only static data, so we must copy it before passing to ImGui
    {
        const float font_size_pixels = 22.0f;
        int font_data_size = static_cast<int>(font_v_size);
        void* font_data_copy = malloc(static_cast<size_t>(font_data_size));
        if (font_data_copy)
        {
            memcpy(font_data_copy, font_v, static_cast<size_t>(font_data_size));
            ...
            if (!font)
            {
                __android_log_print(ANDROID_LOG_ERROR, g_LogTag, "Failed to load embedded font (zt.ttf)");
                free(font_data_copy);
            }
            else
            {
                __android_log_print(ANDROID_LOG_INFO, g_LogTag, "Embedded font loaded successfully");
            }
        }
        else
        {
            __android_log_print(ANDROID_LOG_ERROR, g_LogTag, "Failed to allocate memory for font data");
        }
    }
...
void MainLoopStep()
{
    ...
    // 2. Show the main window with two tabs: "功能" and "设置"
    {
        static float f = 0.0f;
        static int counter = 0;
        static bool anti_screen_record = false; // 新增：防录屏选项
        ...
        if (ImGui::BeginTabBar("MainTabBar"))
        {
            ...
            if (ImGui::BeginTabItem("功能"))
            {
                ...
                ImGui::ColorEdit3("清除颜色", (float*)&clear_color);
            ...
            {
                ImGui::Checkbox("防录屏", &anti_screen_record);
                // 这里只放这一个选项，不加其他内容
                ImGui::EndTabItem();
            }
